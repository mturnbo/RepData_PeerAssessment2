...
the relevant fields is created.
...
Parse the event year and add field to clean dataset.
...
Add fields with the propert and crop damage values multiplied by the exponents.
...
The average value per event is to be used as the factor for determining the weather events with the most impact.

```{r, echo=TRUE, warning=FALSE}
library(data.table)
meanHealthImpact = data.table(aggregate(cbind(FATALITIES, INJURIES) ~ EVTYPE, data=cleanStormData, FUN=mean))
topHealthImpact =  head(meanHealthImpact[order(-meanHealthImpact$FATALITIES , -meanHealthImpact$INJURIES)], 5) 
setnames(topHealthImpact, c("EVTYPE", "FATALITIES", "INJURIES"), c("Event.Type", "Fatalities", "Injuries"))

library(ggplot2)
fig1 <- ggplot(topHealthImpact, aes(x=Event.Type, y=Fatalities))
...
library(ggplot2)
fig2 <- ggplot(topEconomicImpact, aes(x=Event.Type, y=Total.Damage))
fig2 <- fig2 + geom_bar(stat="identity", position=position_dodge())
fig2 <- fig2 + coord_flip()
fig2 <- fig2 + labs(title="Top 5 Costliest Weather Events") + xlab("Event Type") + ylab("Average Impact")
fig2
```

Based on the summary data, we are able to make the following conclusions based on averages per event type:
- **Tornados** cause the most fatalities and economic impact.
