...
# stormData$BGN_DATE_DATE <- as.POSIXct(stormData$BGN_DATE, format = "%m/%d/%Y %h:%M:%S")
...
eventSummary <- data.table(ddply(stormDataUS, c('EVENT_YEAR'), summarise, 
  ...
  min.property.damage = min(PROPDMG),
  max.property.damage = max(PROPDMG),
  avg.property.damage = mean(PROPDMG),
  min.crop.damage = min(CROPDMG),
  max.crop.damage = max(CROPDMG),
  avg.crop.damage = mean(CROPDMG)
))

earliestFatalityYear <- min(eventSummary[which(eventSummary$max.fatalities > 0), EVENT_YEAR])
earliestInjuryYear <- min(eventSummary[which(eventSummary$max.injuries > 0), EVENT_YEAR])
earliestPropertyDamageYear <- min(eventSummary[which(eventSummary$max.property.damage > 0), EVENT_YEAR])
earliestCropDamageYear <- min(eventSummary[which(eventSummary$max.crop.damage > 0), EVENT_YEAR])
```
...
```{r, echo=TRUE}
eventImpactPopulationHealth <- ddply(stormDataUS, c('EVTYPE'), summarise, 
  avgFatality = mean(FATALITIES),
...
```{r, echo=TRUE}
eventImpactDamage <- ddply(stormDataUS, c('EVTYPE'), summarise, 
  avgProperty = mean(PROPDMG),
  avgCrop = mean(CROPDMG)
)
