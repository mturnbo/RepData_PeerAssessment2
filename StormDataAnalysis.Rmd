...
- The downloaded data includes events from `r min(eventSummary$event.year)` through `r max(eventSummary$event.year)`. 
...
eventMaxImpactDamage <- eventImpactDamage[ 
  eventImpactDamage$total.property == max(eventImpactDamage$total.property) |
  eventImpactDamage$avg.property == max(eventImpactDamage$avg.property) |
  eventImpactDamage$total.crop == max(eventImpactDamage$total.crop) |
  eventImpactDamage$avg.crop == max(eventImpactDamage$avg.crop),]

library(reshape2)
meltedImpactPopulationHealth <- melt(eventMaxImpactPopulationHealth, id=c('event.type'), measure.vars=c('total.fatality','avg.fatality','total.injury','avg.injury'))
meltedImpactDamage <- melt(eventMaxImpactDamage, id=c('event.type'), measure.vars=c('total.property','avg.property','total.crop','avg.crop'))

library(ggplot2)
fig1 <- ggplot(data=meltedImpactPopulationHealth, aes(x=variable, y=value, fill=event.type))
fig1 <- fig1 + geom_bar(stat="identity", position=position_dodge())
fig1 <- fig1 + labs(title="U.S. Storm Population Health Impact") + xlab("Event Type") + ylab("Impact ($)")
fig1 <- fig1 + scale_y_log10()
fig1

fig2 <-ggplot(data=meltedImpactDamage, aes(x=variable, y=value, fill=event.type)) 
fig2 <- fig2 + geom_bar(stat="identity", position=position_dodge())
fig2 <- fig2 + labs(title="U.S. Storm Damage Impact") + xlab("Event Type") + ylab("Impact ($)")
fig2 <- fig2 + scale_y_log10()
fig2
```

Based on the summary data, Tornados have the greatest impact on population health and property.  Hail has the most impact on crops.
